...
AmorphousVolume::computeEmissionDistribution(const scene_rdl2::rdl2::VolumeShader* volumeShader) const
{
    ...
    if (emissionGrid->isType<amorphous::ScalarGridType>()) {
        ...
        distribution = computeEmissionDistributionImpl(getRdlGeometry(),
                                                       *static_cast<const amorphous::ScalarGridType*>(emissionGrid.get()),
                                                       mRenderToPrim,
                                                       values,
                                                       volumeShader);
                             ...
                             const scene_rdl2::rdl2::VolumeShader* const) const
{
    ...
    if (mUseAmorphousSampler) {
        ...
        const Color density = mAmorphousSampler.extinct(tls,
                                                        volumeId,
                                                        p,
                                                        Interpolation::POINT);

        Color retVal = density * sampleBakedDensity(tls,
                                                    volumeId,
                                                    p);
                                        ...
                                        const scene_rdl2::rdl2::VolumeShader* const) const
{
    ...
    if (mUseAmorphousSampler) {
        *extinction = mAmorphousSampler.extinct(tls,
                                                volumeId,
                                                p,
                                                mode);

        *albedo = mAmorphousSampler.albedo(tls,
                                           volumeId,
                                           p,
                                           mode);
        if (temperature) {
            *temperature = mAmorphousSampler.incand(tls,
                                                    volumeId,
                                                    p,
                                                    mode);
        }
                                 ...
                                 const Vec3f& pSample) const
{
    ...
    if (mUseAmorphousSampler) {
        return mAmorphousSampler.incand(tls,
                                        volumeId,
                                        p,
                                        Interpolation::POINT);
    } else {
...
{
    const float emissionSampleRate = scene_rdl2::math::clamp(mVdbVolumeData->mEmissionSampleRate);
