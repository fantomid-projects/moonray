...
class TriMesh : public PolyMesh
{
public:
    TriMesh(size_t estiFaceCount,
    ...
    virtual bool bakePosMap(int width,
                            ...
                            Vec3fa *posResult,
                            Vec3f *nrmResult) const override;
    ...
    virtual scene_rdl2::math::Vec3f getVelocity(int tessFaceId,
                                                int vIndex,
                                                float time = 0.0f) const override
    {
        int baseFaceId = getBaseFaceId(tessFaceId);
        return getAttribute(shading::StandardAttributes::sVelocity,
                            baseFaceId,
                            tessFaceId,
                            vIndex,
                            time);
...
private:
    ...
    virtual void generateIndexBufferAndSurfaceSamples(const std::vector<PolyFaceTopology>& faceTopologies,
    ...
    template <typename T>
    bool getTriangleAttributes(const shading::TypedAttributeKey<T>& key,
                               int baseFaceId,
                               int tessFaceId,
                               T& v1, T& v2, T& v3,
                               float time,
    ...
    void getTriangleST(int baseFaceId,
                       int tessFaceId,
                       Vec2f& st1, Vec2f& st2, Vec2f& st3,
                       bool isFirst) const;

    void getTriangleNormal(int baseFaceId,
                           int tessFaceId,
