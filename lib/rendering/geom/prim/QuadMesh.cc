                     ...
                     Vec3f *nrmResult) const
{
    ...
    for (size_t faceId = 0; faceId < getTessellatedMeshFaceCount(); ++faceId) {
        ...
        if (udimxform(udim, st0) && udimxform(udim, st1) &&
            ...
            scene_rdl2::math::BBox2f roiST(Vec2f(std::min(st0.x, std::min(st1.x, st3.x)),
...
void
QuadMesh::setRequiredAttributes(int primId,
                                float time,
                                ...
                                Intersection& intersection) const
{
    ...
    if (mIsTessellated) {
        ...
        Vec2f uv = vertexW[0] * mFaceVaryingUv[sQuadVertexCount * tessFaceId    ] +
    ...
    MeshInterpolator interpolator(primitiveAttributes,
                                  ...
                                  varyingW[0], varyingW[1], varyingW[2], varyingW[3],
                                  tessFaceId,
                                  id1, id2, id3, id4,
                        ...
                        Intersection& intersection) const
{
    ...
    if (!hasExplicitAttributes) {
        ...
        if (isMotionBlurOn()) {
            ...
            Vec3f dp0[2];
            if (!computeTrianglePartialDerivatives(p10, p20, p30,
                                                   st1, st2, st3,
                                                   dp0)) {
...
void
QuadMesh::getNeighborVertices(int baseFaceId,
                              int tessFaceId,
                              int tessFaceVIndex,
...
void
QuadMesh::generateIndexBufferAndSurfaceSamples(const std::vector<PolyFaceTopology>& quadTopologies,
    ...
    for (size_t f = 0; f < quadTopologies.size(); ++f) {
        ...
        for (int i = 0; i < interiorRowVertexCount - 1; ++i) {
            for (int j = 0; j < interiorColVertexCount - 1; ++j) {
                indices.push_back(tessellatedVertexLookup.getInteriorVertexId(f, i, j,
                                                                              interiorColVertexCount));

                indices.push_back(tessellatedVertexLookup.getInteriorVertexId(f, i, j + 1,
                                                                              interiorColVertexCount));

                indices.push_back(tessellatedVertexLookup.getInteriorVertexId(f, i + 1, j + 1,
                                                                              interiorColVertexCount));

                indices.push_back(tessellatedVertexLookup.getInteriorVertexId(f, i + 1, j,
                                                                              interiorColVertexCount));
                if (faceVaryingUv) {
                    faceVaryingUv->emplace_back((j + 1) * du, (i + 1) * dv);
        ...
        for (int i = 0; i < interiorRowVertexCount; ++i) {
            for (int j = 0; j < interiorColVertexCount; ++j) {
                int vid = tessellatedVertexLookup.getInteriorVertexId(f, i, j,
                                                                      interiorColVertexCount);
                surfaceSamples[vid].mFaceId = f;
        ...
        //    / \ / \ | / \ / \
        //   *---*----*----*---*
        const Vec2f cornerUv[4] = { Vec2f(0.0f, 0.0f), Vec2f(1.0f, 0.0f),
...
QuadMesh::generateVertexBuffer(const PolygonMesh::VertexBuffer& baseVertices,
...
void
QuadMesh::fillDisplacementAttributes(int tessFaceId,
                                     int vIndex,
                                     Intersection& intersection) const
{
                       ...
                       float time) const
{
    ...
    case RATE_VARYING:
        ...
        if (mIsTessellated) {
            ...
            T a4 = getVaryingAttribute(key,
                                       mBaseIndices[baseOffset + 3],
                                       time);

            result = bilinearInterpolate(uv, a1, a2, a3, a4);
        } else {
            result = getVaryingAttribute(key,
                                         mIndices[sQuadVertexCount * baseFaceId + vIndex],
                                         time);
        }
        }
        break;
    case RATE_FACE_VARYING:
        ...
        if (mIsTessellated) {
            ...
            T a4 = getFaceVaryingAttribute(key,
                                           baseFaceId,
                                           3,
                                           time);
...
void
QuadMesh::getQuadST(int baseFaceId,
                    int tessFaceId,
...
void
QuadMesh::getQuadNormal(int baseFaceId,
                        int tessFaceId,
