...
protected:
    ...
    int getBaseFaceId(int tessellatedFaceId) const
    {
        return mIsTessellated ?
               mTessellatedToBaseFace[tessellatedFaceId] :
               tessellatedFaceId;
    }
    ...
    virtual void generateIndexBufferAndSurfaceSamples(const std::vector<PolyFaceTopology>& quadTopologies,
    ...
    virtual PolygonMesh::VertexBuffer generateVertexBuffer(const PolygonMesh::VertexBuffer& baseVertices,
    ...
    bool shouldTessellate(bool enableDisplacement,
                          const scene_rdl2::rdl2::Layer *pRdlLayer) const;

    std::vector<PolyTessellationFactor> computeTessellationFactor(const scene_rdl2::rdl2::Layer *pRdlLayer,
    ...
    std::vector<PolyFaceTopology> generatePolyFaceTopology(const PolyTopologyIdLookup& topologyIdLookup) const;
    ...
    getFaceVaryingAttribute(shading::TypedAttributeKey<T> key,
                            int fid,
                            int vIndex,
                            float time = 0.0f) const
    {
        const shading::Attributes* attributes = getAttributes();
        MNRY_ASSERT(attributes->isSupported(key) && attributes->getRate(key) == shading::RATE_FACE_VARYING);
    ...
    computeAttributeDerivatives(shading::TypedAttributeKey<T> key,
                                ...
                                uint32_t vid1, uint32_t vid2, uint32_t vid3,
                                float time,
    ...
    virtual void getNeighborVertices(int baseFaceId,
                                     int tessFaceId,
                                     int tessFaceVIndex,
                                     int& vid, int& vid1, int& vid2, int& vid3,
                                     Vec2f& st, Vec2f& st1,
                                     Vec2f& st2, Vec2f& st3) const = 0;
    ...
    void* getBakedAttributeData(const shading::TypedAttributeKey<T>& key,
                                size_t& numElements,
                                shading::AttributeRate &newRate) const;
