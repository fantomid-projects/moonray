...
PolyTopologyIdLookup::PolyTopologyIdLookup(const int vertexCount,
                                           const int faceVertexCount,
                                           const PolygonMesh::IndexBuffer& indices):
    TopologyIdLookup(vertexCount)
{
    MNRY_ASSERT_REQUIRE(faceVertexCount == sQuadVertexCount || faceVertexCount == sTriangleVertexCount);
    ...
    mQuadrangulatedFaceCount(0)
{
    ...
    if (noTessellation) {
        ...
        mSplitVertexCount = mCoarseVertexCount + mCoarseEdgeCount + mCoarseFaceCount;
...
SubdTopologyIdLookup::SubdTopologyIdLookup(const int faceVaryingVertexCount,
                                           ...
                                           const SubdivisionMesh::IndexBuffer& faceVaryingIndices,
                                           bool noTessellation) :
    SubdTopologyIdLookup(faceVaryingVertexCount,
                         faceVertexCount,
                         faceVaryingIndices,
                         noTessellation)
{
    ...
    for (const auto& vertices : overlapVertices) {
        if (vertices.second.size() > 1) {
            auto& overlapVertices = mFaceVaryingSeams.mOverlapVertices[vertices.first];
...
PolyTessellatedVertexLookup::PolyTessellatedVertexLookup(const std::vector<PolyFaceTopology>& faceTopologies,
        ...
        if (faceVertexCount == sQuadVertexCount) {
            ...
            if (maxEdgeVertexCount != 0) {
                ...
                if (maxEdgeVertexCount % 2 == 0) {
                    interiorVertexCount = 3 * maxEdgeVertexCount * maxEdgeVertexCount / 4;

                    // 1 for the central triangle
                    // the rest is a (n + 0) * h / 2 formula
                    // where h = (n + 2) / 2
                    mEstimatedFaceCount += 1 + 3 * ((maxEdgeVertexCount * maxEdgeVertexCount) / 4 + maxEdgeVertexCount / 2);
                } else {
                    interiorVertexCount = 1 + 3 * (maxEdgeVertexCount * maxEdgeVertexCount - 1) / 4;
                    ...
                    // (1 + n) * h / 2
                    // where h = (n + 1) / 2
                    mEstimatedFaceCount += 3 * (maxEdgeVertexCount * maxEdgeVertexCount +
...
int
PolyTessellatedVertexLookup::getInteriorRowVertexCount(const PolyFaceTopology& faceTopology) const
{
    MNRY_ASSERT(mFaceVertexCount == sQuadVertexCount);
    int interiorRowVertexCount = scene_rdl2::math::max(mOuterEdgeVertexIdsCount[faceTopology.mEdgeId[1]],
                                                       mOuterEdgeVertexIdsCount[faceTopology.mEdgeId[3]]);
    return scene_rdl2::math::max(1, interiorRowVertexCount);
}

int
PolyTessellatedVertexLookup::getInteriorColVertexCount(const PolyFaceTopology& faceTopology) const
{
    MNRY_ASSERT(mFaceVertexCount == sQuadVertexCount);
    int interiorColVertexCount = scene_rdl2::math::max(mOuterEdgeVertexIdsCount[faceTopology.mEdgeId[0]],
                                                       mOuterEdgeVertexIdsCount[faceTopology.mEdgeId[2]]);
    return scene_rdl2::math::max(1, interiorColVertexCount);
}

int
PolyTessellatedVertexLookup::getMaxEdgeVertexCount(const PolyFaceTopology& faceTopology) const
{
    MNRY_ASSERT(mFaceVertexCount == sTriangleVertexCount);
    return scene_rdl2::math::max(scene_rdl2::math::max(getEdgeVertexCount(faceTopology.mEdgeId[0]),
                                                       getEdgeVertexCount(faceTopology.mEdgeId[1])),
                                 getEdgeVertexCount(faceTopology.mEdgeId[2]));
}

int
PolyTessellatedVertexLookup::getTriangleInteriorVertexId(int face,
                                                         ...
                                                         int n) const
{
    ...
    if (interiorRingIndex > 0) {
        offset += 3 * (maxEdgeVertexCount - interiorRingIndex) * interiorRingIndex;
    }
    int vertexPerEdge = maxEdgeVertexCount - 2 * interiorRingIndex;

    // otherwise it is a degened single vertex ring
    if (vertexPerEdge > 1) {
        offset += (cornerIndex * (vertexPerEdge - 1) + n) % (3 * (vertexPerEdge - 1));
    }
    return mInteriorVertexIdsStart[face] + offset;
}

int
PolyTessellatedVertexLookup::getTriangleRingCount(const PolyFaceTopology& faceTopology) const
{
    MNRY_ASSERT(mFaceVertexCount == sTriangleVertexCount);
...
bool
PolyTessellatedVertexLookup::noRingToStitch(const PolyFaceTopology& faceTopology) const
{
    if (mFaceVertexCount == sQuadVertexCount) {
        ...
        return edgeVertexCount0 == edgeVertexCount2 && edgeVertexCount1 == edgeVertexCount3;
    } else {
        int edgeVertexCount0 = getEdgeVertexCount(faceTopology.mEdgeId[0]);
        int edgeVertexCount1 = getEdgeVertexCount(faceTopology.mEdgeId[1]);
        int edgeVertexCount2 = getEdgeVertexCount(faceTopology.mEdgeId[2]);
        return edgeVertexCount0 == edgeVertexCount1 && edgeVertexCount0 == edgeVertexCount2;
    }
}

SubdTessellatedVertexLookup::SubdTessellatedVertexLookup(const std::vector<SubdQuadTopology>& quadTopologies,
    ...
    for (size_t f = 0; f < quadTopologies.size(); ++f) {
        ...
        if (quadTopology.nonQuadParent()) {
            ...
            for (size_t i = 0; i < sQuadVertexCount; ++i) {
                ...
                int controlEdgeResolution = getEdgeVertexCount(eid0) + getEdgeVertexCount(eid1) + 2;
                if (controlEdgeResolution > mMaxEdgeResolution) {
                    mMaxEdgeResolution = controlEdgeResolution;
                }
            }
        ...
        int interiorVertexCount = getInteriorRowVertexCount(quadTopology) * getInteriorColVertexCount(quadTopology);
        mInteriorVertexIdsStart[f] = mTotalVertexCount;
        mTotalVertexCount += interiorVertexCount;
    }
}
...
int
SubdTessellatedVertexLookup::getInteriorRowVertexCount(const SubdQuadTopology& quadTopology) const
{
    int interiorRowVertexCount;
    if (quadTopology.nonQuadParent()) {
        interiorRowVertexCount = scene_rdl2::math::max(mOuterEdgeVertexIdsCount[quadTopology.mEdgeId0[1]],
                                                       mOuterEdgeVertexIdsCount[quadTopology.mEdgeId0[3]]);
    } else {
        interiorRowVertexCount = 1 + scene_rdl2::math::max((mOuterEdgeVertexIdsCount[quadTopology.mEdgeId0[1]] +
...
int
SubdTessellatedVertexLookup::getInteriorColVertexCount(const SubdQuadTopology& quadTopology) const
{
    int interiorColVertexCount;
    if (quadTopology.nonQuadParent()) {
        interiorColVertexCount = scene_rdl2::math::max(mOuterEdgeVertexIdsCount[quadTopology.mEdgeId0[0]],
                                                       mOuterEdgeVertexIdsCount[quadTopology.mEdgeId0[2]]);
    } else {
        interiorColVertexCount = 1 + scene_rdl2::math::max((mOuterEdgeVertexIdsCount[quadTopology.mEdgeId0[0]] +
...
bool
SubdTessellatedVertexLookup::noRingToStitch(const SubdQuadTopology& quadTopology) const
{
    if (quadTopology.nonQuadParent()) {
                        ...
                        int& outerRingVertexCount)
{
    ...
    if (cornerIndex == 0) {
        for (int i = 0; i < innerRingVertexCount; ++i) {
            int row = 0;
            int col = i;
            innerRing[i].mVertexId = tessellatedVertexLookup.getInteriorVertexId(fid, row, col,
        ...
        for (int i = 0; i < innerRingVertexCount; ++i) {
            ...
            innerRing[i].mVertexId =
                tessellatedVertexLookup.getInteriorVertexId(fid, row, col,
                                                            interiorColVertexCount);
            innerRing[i].mT = (i + 1) * du;
            innerRing[i].mDeltaT = du;
        }
    ...
    if (quadTopology.nonQuadParent()) {
        ...
        int edgeVertexCount = tessellatedVertexLookup.getEdgeVertexCount(edgeId);
        ...
        int vidStart = tessellatedVertexLookup.getTessellatedVertexId(quadTopology.mCornerVertexId[cornerIndex]);
        int vidEnd = tessellatedVertexLookup.getTessellatedVertexId(quadTopology.mCornerVertexId[(cornerIndex + 1) % sQuadVertexCount]);
        ...
        for (int i = 0; i < edgeVertexCount; ++i) {
            outerRing[i + 1].mVertexId = tessellatedVertexLookup.getEdgeVertexId(edgeId, i, endAtEdgeMiddle);
        ...
        int edgeVertexCount0 = tessellatedVertexLookup.getEdgeVertexCount(edgeId0);
        int edgeVertexCount1 = tessellatedVertexLookup.getEdgeVertexCount(edgeId1);
        ...
        for (int i = 0; i < edgeVertexCount0; ++i) {
            ...
            outerRing[offset + i].mVertexId = tessellatedVertexLookup.getEdgeVertexId(edgeId0,
                                                                                      i, true);
        ...
        for (int i = 0; i < edgeVertexCount1; ++i) {
            ...
            outerRing[offset + i].mVertexId = tessellatedVertexLookup.getEdgeVertexId(edgeId1,
                                                                                      i, false);
                        ...
                        int& outerRingVertexCount)
{
    ...
    if (cornerIndex == 0) {
        for (int i = 0; i < innerRingVertexCount; ++i) {
            int row = 0;
            int col = i;
            innerRing[i].mVertexId = tessellatedVertexLookup.getInteriorVertexId(fid,
                                                                                 row,
                                                                                 col,
                                                                                 interiorColVertexCount);
            innerRing[i].mT = (i + 1) * du;
    ...
    for (int i = 0; i < edgeVertexCount; ++i) {
        outerRing[offset + i].mVertexId = tessellatedVertexLookup.getEdgeVertexId(edgeId, i, reverseEdge);
                            ...
                            int& outerRingVertexCount)
{
    ...
    const Vec2f cornerUv0[3] = { Vec2f(0.0f, 0.0f),
                                 Vec2f(1.0f, 0.0f),
                                 Vec2f(0.0f, 1.0f) };

    const Vec2f cornerUv1[3] = { Vec2f(ringUvDelta, ringUvDelta),
                                 Vec2f(1.0f - 2 * ringUvDelta, ringUvDelta),
                                 Vec2f(ringUvDelta, 1.0f - 2 * ringUvDelta)};

    // allocate and initialize inner ring
    innerRingVertexCount = maxEdgeVertexCount;
    float wInner = innerRingVertexCount == 1 ? 0.0f: 1.0f / (float)(innerRingVertexCount - 1);
    innerRing = arena->allocArray<StitchPoint>(innerRingVertexCount);
    innerRingUv = arena->allocArray<Vec2f>(innerRingVertexCount);
    for (int i = 0; i < innerRingVertexCount; ++i) {
        innerRing[i].mVertexId = tessellatedVertexLookup.getTriangleInteriorVertexId(fid, maxEdgeVertexCount,
                                                                                     0, cornerIndex, i);
        innerRing[i].mT = (i + 1) * innerDeltaT;
        innerRing[i].mDeltaT = innerDeltaT;
        innerRingUv[i] = Vec2f((1.0f - i * wInner) * cornerUv1[index0] +
    ...
    int vidStart = tessellatedVertexLookup.getTessellatedVertexId(faceTopology.mCornerVertexId[index0]);
    int vidEnd = tessellatedVertexLookup.getTessellatedVertexId(faceTopology.mCornerVertexId[index1]);
    bool reverseEdge = faceTopology.mCornerVertexId[index0] > faceTopology.mCornerVertexId[index1];
    ...
    for (int i = 0; i < edgeVertexCount; ++i) {
        ...
        outerRingUv[offset + i] = (1.0f - (i + 1) * wOuter) * cornerUv0[index0] +
                                  ...
                                  int& colVertexCount)
{
    ...
    bool reverseEdge0 = quadTopology.mCornerVertexId[0] > quadTopology.mCornerVertexId[1];
    bool reverseEdge1 = quadTopology.mCornerVertexId[1] > quadTopology.mCornerVertexId[2];
    bool reverseEdge2 = quadTopology.mCornerVertexId[2] > quadTopology.mCornerVertexId[3];
    bool reverseEdge3 = quadTopology.mCornerVertexId[3] > quadTopology.mCornerVertexId[0];
    ...
    // bottom row
    vids[offset++] = tessellatedVertexLookup.getTessellatedVertexId(quadTopology.mCornerVertexId[0]);
    for (int i = 1; i < colVertexCount - 1; ++i) {
        vids[offset++] = tessellatedVertexLookup.getEdgeVertexId(eid0,
                                                                 i - 1,
                                                                 reverseEdge0);
    }
    vids[offset++] = tessellatedVertexLookup.getTessellatedVertexId(quadTopology.mCornerVertexId[1]);

    // middle
    for (int i = 1; i < rowVertexCount - 1; ++i) {
        vids[offset++] = tessellatedVertexLookup.getEdgeVertexId(eid3,
                                                                 i - 1,
                                                                 !reverseEdge3);
        for (int j = 1; j < colVertexCount - 1; ++j) {
            vids[offset++] = tessellatedVertexLookup.getInteriorVertexId(baseFaceId,
                                                                         i - 1,
                                                                         j - 1,
                                                                         colVertexCount - 2);
        }
        vids[offset++] = tessellatedVertexLookup.getEdgeVertexId(eid1,
                                                                 i - 1,
                                                                 reverseEdge1);
    }

    // top row
    vids[offset++] = tessellatedVertexLookup.getTessellatedVertexId(quadTopology.mCornerVertexId[3]);
    for (int i = 1; i < colVertexCount - 1; ++i) {
        vids[offset++] = tessellatedVertexLookup.getEdgeVertexId(eid2,
                                                                 i - 1,
                                                                 !reverseEdge2);
    }
    vids[offset++] = tessellatedVertexLookup.getTessellatedVertexId(quadTopology.mCornerVertexId[2]);
}
