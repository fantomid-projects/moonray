...
// This directive should be commented out for the release version.
...
CpuAffinityManager::configureCpuAffinity()
{
    ...
    std::cerr << ">> AffinityManager.cc CpuAffinityManager::configureCpuAffinity() start {\n"
              << "  mCpuAffinityDef:" << mCpuAffinityDef << '\n'
              << "  mSocketAffinityDef:" << mSocketAffinityDef << '\n'
              << "}\n";
#endif // end DEBUG_MSG_INIT
    ...
    if (!forceToDisable) {
        if (mNumThreads == std::thread::hardware_concurrency()) {
            ...
            // This is the case of MCRT CPU Affinity disabled
            ostr << "MCRT-CPU-affinity control disabled : numRenderThreads:" << mNumThreads;
        }
    } else {
        //
        // force to disable MCRT CPU Affinity
        //
        ostr << "MCRT-CPU-affinity control disabled : numRenderThreads:" << mNumThreads;
    }
    mMcrtMessage = ostr.str();
...
#ifdef DEBUG_MSG_INIT
    std::cerr << ">> AffinityManager.cc AffinityManager::init() {\n"
              << "  autoAffinityDef:" << autoAffinityDef << '\n'
              << "  cpuAffinityDef:" << cpuAffinityDef << '\n'
              << "  socketAffinityDef:" << socketAffinityDef << '\n'
              << "  memAffinityDef:" << memAffinityDef << '\n'
              << "}\n";
#endif // end DEBUG_MSG_INIT
...
#ifdef DEBUG_MSG_INIT
        std::cerr << ">> AffinityManager.cc AffinityManager before construction (auto=ON) {\n"
                  << "  autoCpuAffinityDef:" << autoCpuAffinityDef << '\n'
                  << "  autoMemAffinityDef:" << autoMemAffinityDef << '\n'
                  << "}\n";
#endif // end DEBUG_MSG_INIT
...
#ifdef DEBUG_MSG_INIT 
        std::cerr << ">> AffinityManager.cc AffinityManager before construction (auto=OFF) {\n"
                  << "  cpuAffinityDef:" << cpuAffinityDef << '\n'
                  << "  socketAffinityDef:" << socketAffinityDef << '\n'
                  << "  memAffinityDef:" << memAffinityDef << '\n'
                  << "}\n";
